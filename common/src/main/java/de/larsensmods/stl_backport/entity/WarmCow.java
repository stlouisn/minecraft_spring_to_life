...
public class WarmCow extends Cow {
    ...
    protected void registerGoals() {
        super.registerGoals();
        this.goalSelector.addGoal(2, new BreedGoal(this, 1.0, ColdCow.class));
    ...
    public @Nullable Cow getBreedOffspring(ServerLevel level, AgeableMob otherParent) {
        ...
        if(otherCow instanceof ColdCow) {
            return Math.random() < 0.5 ? STLEntityTypes.COLD_COW.get().create(level) : STLEntityTypes.WARM_COW.get().create(level);
        }else if(otherCow instanceof WarmCow) {
            return STLEntityTypes.WARM_COW.get().create(level);
        }else{
            return Math.random() < 0.5 ? STLEntityTypes.WARM_COW.get().create(level) : EntityType.COW.create(level);
        }
    }
}
